...
#SBATCH --output=ct_vitl_%A_%a.out
#SBATCH --error=ct_vitl_%A_%a.err
...
bazelisk run :start_data_producer_vector -- \
  ...
  >$SCRATCH/producer_vitl16.out 2>$SCRATCH/producer_vitl16.err &
...
srun --export=ALL python fomo/ct/train/train_2d.py \
  --config-file fomo/ct/configs/train/2d/ct_vitl_16_no_pretrain.yaml \
  --output-dir $SCRATCH/dino_output/2d/ct_vitl_16_$date_now \
  --no-resume
