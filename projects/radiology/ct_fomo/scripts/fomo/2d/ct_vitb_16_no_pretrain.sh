...
#SBATCH --output=ct_vitb_%A_%a.out
#SBATCH --error=ct_vitb_%A_%a.err
...
bazelisk run :start_data_producer_vector -- \
  ...
  >$SCRATCH/producer_vitb16.out 2>$SCRATCH/producer_vitb16.err &
...
srun --export=ALL python fomo/ct/train/train_2d.py \
  --config-file fomo/ct/configs/train/2d/ct_vitb_16_no_pretrain.yaml \
  --output-dir $SCRATCH/dino_output/2d/ct_vitb_16_$date_now \
  --no-resume
