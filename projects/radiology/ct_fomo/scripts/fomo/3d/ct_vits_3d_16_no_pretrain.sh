...
#SBATCH --output=ct_vits_3d_%A_%a.out
#SBATCH --error=ct_vits_3d_%A_%a.err
...
bazelisk run :start_data_producer_vector -- \
  --name ct_vector_dataset --producer-type ct --chunk-size-mb 33 --max-memory-size-gb 700 \
  --num-workers 12 --database-url $DATABASE_URL \
  --slices-per-chunk 32 >$SCRATCH/producer_vits16.out 2>$SCRATCH/producer_vits16.err &
...
srun --export=ALL python fomo/ct/train/train3d.py \
  --config-file fomo/ct/configs/train/3d/ct_vits_3d_16_no_pretrain.yaml \
  --output-dir $SCRATCH/dino_output/3d/ct_vits_16_$date_now \
  --no-resume
