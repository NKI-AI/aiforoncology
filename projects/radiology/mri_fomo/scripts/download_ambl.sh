...
#SBATCH --job-name=download_abml
#SBATCH --partition=cpu
#SBATCH --nodelist=gaia
#SBATCH --cpus-per-task=1
#SBATCH --qos=cpu_qos
#SBATCH --time=3-00:00:00
#SBATCH --output=/home/v.v.veldhuizen/slurm_output/download_ambl_%A.out
#SBATCH --error=/home/v.v.veldhuizen/slurm_output/download_ambl_%A.err
...
OUTPUT_DIR="/data/groups/public/archive/radiology/ambl"
mkdir -p $OUTPUT_DIR
...
download_patient() {
  ...
  for study_uid in $study_uids; do
    ...
    local series_url="$NBIA_BASE_URL/TCIA/query/getSeries?Collection=$COLLECTION&StudyInstanceUID=$study_uid&format=json"
    local series_response=$(curl -s "$series_url")

    if [ -z "$series_response" ]; then
      log "ERROR: No series found for study $study_uid"
      continue
    fi
...
log "Starting download of $COLLECTION collection"
