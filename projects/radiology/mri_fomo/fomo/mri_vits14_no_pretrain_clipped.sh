...
#SBATCH --job-name=mri_vits_pretrain
#SBATCH --gres=gpu:4
...
#SBATCH --output=mri_vits_clip_%A_%a.out
#SBATCH --error=mri_vits_clip_%A_%a.err
...
bazelisk run :start_data_producer_vector -- \
  ...
  >/processing/$current_user/producer_vits_clip.out 2>/processing/$current_user/producer_vits_clip.err &
...
srun --export=ALL bazelisk run :dinov2_train_mri -- \
  --config-file fomo/mri/configs/train/mri_vits14_no_pretrain_grad_clip.yaml \
