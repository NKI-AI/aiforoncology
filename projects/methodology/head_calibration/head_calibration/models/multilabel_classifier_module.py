...
class MultilabelClassifierLitModule(LightningModule):
    def __init__(
        self,
        num_classes: int,
        net: torch.nn.Module,
    ...
    def model_step(self, batch: Tuple[torch.Tensor, torch.Tensor]) -> Tuple[dict, torch.Tensor, torch.Tensor]:
        ...
        class_idx_oh = torch.nn.functional.one_hot(class_idx, num_classes=self.num_classes).float()
        loss_input_dict = {"labels": class_idx_oh}

        loss_dict = self.loss(logits, loss_input_dict)
        return loss_dict, probs, class_idx_oh
