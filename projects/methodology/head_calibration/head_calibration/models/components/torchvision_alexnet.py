...
__all__ = ["AlexNet", "AlexNet_Weights", "calexnet"]


class AlexNet(nn.Module):
    def __init__(self, num_classes: int = 1000, dropout: float = 0.5, num_channels: int = 3) -> None:
        super().__init__()
...
def calexnet(*, weights: Optional[AlexNet_Weights] = None, progress: bool = True, **kwargs: Any) -> AlexNet:
    ...
    if weights is not None:
        # check whether num_channels is in kwargs
        if "num_channels" in kwargs:
            # get the num_channels
            num_channels = kwargs["num_channels"]
            if num_channels != 3:
                # if num_channels is not 3, raise ValueError
                raise ValueError("num_channels must be 3 when using pretrained weights")
