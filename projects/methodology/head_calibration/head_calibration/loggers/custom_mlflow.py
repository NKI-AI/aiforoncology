...
class CMLFlowLogger(Logger):
    ...
    def experiment(self) -> "MlflowClient":
        ...
        if self._run_id is None:
            ...
            run = self._mlflow_client.create_run(experiment_id=self._experiment_id, tags=resolve_tags(self.tags))  # type: ignore[attr-defined]
    ...
    def log_metrics(self, metrics: Mapping[str, float], step: Optional[int] = None) -> None:
        ...
        metrics = _add_prefix(metrics, self._prefix, self.LOGGER_JOIN_CHAR)  # type: ignore[attr-defined]
    ...
    def finalize(self, status: str = "success") -> None:
        ...
        if self._checkpoint_callback:
            ...
            self._scan_and_log_state_dicts(self._checkpoint_callback)
    ...
    def after_save_checkpoint(self, checkpoint_callback: ModelCheckpoint) -> None:
        ...
        if self._log_model == "all" or self._log_model is True and checkpoint_callback.save_top_k == -1:
            ...
            self._scan_and_log_state_dicts(checkpoint_callback)
    ...
    def _scan_and_log_state_dicts(self, checkpoint_callback: ModelCheckpoint) -> None:
        # get checkpoints to be saved with associated score
        checkpoints = _scan_checkpoints(checkpoint_callback, self._logged_model_time)
        ...
        for t, p, s, tag in checkpoints:
            ...
            # Artifact path on mlflow
            artifact_path = f"model/state_dicts/{Path(p).stem}"

            # # Log the checkpoint
            # self.experiment.log_artifact(self._run_id, p, artifact_path)

            # THIS IS THE CUSTOM PART
            # It removes everything except the state_dict
            # Load the checkpoint and keep only the state_dict
            ...
            with tempfile.TemporaryDirectory(prefix="test", suffix="test", dir=os.getcwd()) as tmp_dir:
                ...
                # Log the state_dict
                torch.save(state_dict, f"{tmp_dir}/state_dict.pth")
...
def _get_resolve_tags() -> Callable:
    ...
    if hasattr(context, "registry"):
        ...
        def resolve_tags(tags: Dict[str, Any]) -> Dict[str, Any]:
            return tags
