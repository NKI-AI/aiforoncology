...
def plot_line(x, y, colors, ax, **kwargs):
    ...
    lc = LineCollection(segments, colors=colors, linestyle="solid", joinstyle="round", **kwargs)
...
def prepare_rel_diagram_binned(f, y, nbins=15):
    ...
    alphas = np.array([bs / np.sum(binning.bucket_sizes_) for bs in binning.bucket_sizes_])
...
def plot_rel_diagram_binned(t, mu, buckets, alphas, ece, fig=None, ax=None):
    ...
    ece_label = f"$\\mathrm{{ECE}}_{{{nbins}}}: {ece:.3f}$" if config.use_tex_fonts else f"ECE$_{{{nbins}}}: {ece:.3f}$"
...
def prepare_rel_diagram(
    ...
    **unused_kwargs,
    ...
    if plot_bag_lines or plot_confidence_band:
        bag = BaggingRegressor(estimator=estimator, n_estimators=num_bootstrap).fit(f.reshape(-1, 1), y)
        dfs = []
...
def plot_rel_diagram(
    ...
    **unused_kwargs,
    ...
    for mu in diagram["bags"]:
        alpha_scale = 0.20
        plot_line(t, mu, ax=ax, colors=[np.clip(color * di * alpha_scale, 0, 1) for di in density], lw=1)
    ...
    if special_endpoints:
        for x0 in [0, 1]:
            ...
            ax.text(
                x=x0 + text_shift, y=mu, s=f"({pt_dens:.2f})", fontsize="x-small", va="center", ha=ha, clip_on=False
            )
