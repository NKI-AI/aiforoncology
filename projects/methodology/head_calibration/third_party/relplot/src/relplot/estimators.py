...
class Binning(BaseEstimator):
    ...
    def fit(self, data, labels):
        ...
        self.bucket_predictions_ = np.array([b / bs if bs > 0 else 0 for (b, bs) in zip(buckets, bucket_sizes)])
...
class RecalibratedEstimator(BaseEstimator):
    ...
    def _sk_visual_block_(self):
        estimation_block = _VisualBlock("parallel", [self.estimator], names=["estimator"], dash_wrapped=False)
        recalibration_block = _VisualBlock(
            "parallel",
            [self.recalibration_regressor],
            names=[f"recalibration_regressor: {self.recalibration_regressor.__class__.__name__}"],
            dash_wrapped=False,
        )
        return _VisualBlock("serial", [estimation_block, recalibration_block], dash_wrapped=False)
