...
warnings.filterwarnings("default", category=DeprecationWarning, module="relplot.kernel_utils")
...
def kernel_smooth_basic(x, y, x_eval, kernel_func):
    ...
    ys = np.divide(Ktx.dot(y), dens, out=np.zeros_like(t), where=dens != 0)  # handle potential divide-by-zeros
    return ys
...
class BaseKernelMixin:
    ...
    def kde(self, x, x_eval, debias_boundary=True, bounds=(0, 1), eps=0.00001):
