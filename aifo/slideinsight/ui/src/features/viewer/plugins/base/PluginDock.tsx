...
export function PluginDock({ className }: { className?: string }) {
  ...
  useEffect(() => {
    const updatePlugins = () => {
      const toolbarPlugins = pluginManager.getToolbarPlugins();
      console.log(
        "PluginDock: Found toolbar plugins:",
        toolbarPlugins.map((p) => p.id)
      );
      setPlugins(toolbarPlugins);
    ...
    const setupStateSubscriptions = (plugins: IPlugin[]) => {
  ...
  const handlePluginClick = useCallback((pluginId: string) => {
    pluginManager.handleButtonClick(pluginId);
  }, []);
...
export function PluginRenderer({
  ...
  const [openPanels, setOpenPanels] = useState<
    Array<{ plugin: IPlugin; state: PluginState }>
  >([]);
    ...
    <>
      ...
      {(renderMode === "all" || renderMode === "docked") &&
        dockedPanels.map(({ plugin, state }) => {
          const PanelComponent = plugin.getPanelComponent();
...
export function useDockedPlugins() {
  ...
  useEffect(() => {
    ...
    const unsubscribePluginChanges =
      pluginManager.subscribeToPluginChanges(updateDockedInfo);
