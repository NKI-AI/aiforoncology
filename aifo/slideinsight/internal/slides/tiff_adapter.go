...
func NewTiffAdapter(path string) (Slide, error) {
	...
	if err == nil {
		mppXStr := strconv.FormatFloat(mppX, 'f', -1, 64)
		mppYStr := strconv.FormatFloat(mppY, 'f', -1, 64)
		// Set generalized property names only
		adapter.properties["mpp-x"] = mppXStr
		adapter.properties["mpp-y"] = mppYStr
	}
...
func (a *TiffAdapter) ReadRegion(x, y, level, w, h int) (image.Image, error) {
	...
	// Coordinates are already in level-native coordinate system (no conversion needed)
...
func (a *TiffAdapter) Mpp() (float64, float64, error) {
	mppX, mppY, err := a.tiff.BaseResolution()
	if err != nil {
		return 0, 0, err
	}
...
// TIFF adapter doesn't support spectral conversion, so this falls back to regular ReadRegion.
func (a *TiffAdapter) ReadRegionAsFastslideImage(x, y, level, w, h int) (*fastslide.Image, error) {
...
// IsSpectral returns false since basic TIFF adapter doesn't handle spectral slides.
func (a *TiffAdapter) IsSpectral() bool {
	return false
}

// GetChannelMetadata returns nil since TIFF adapter doesn't support spectral metadata.
func (a *TiffAdapter) GetChannelMetadata() ([]ChannelMetadata, error) {
	return nil, nil
}
