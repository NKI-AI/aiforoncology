...
func HandleError(c *fiber.Ctx, err error) error {
	...
	case errors.IsPasswordResetRequired(err):
		return SendError(c, fiber.StatusLocked, err.Error())
	case errors.IsAccountInactive(err):
		return SendError(c, fiber.StatusForbidden, err.Error())
	case errors.IsInvalidResetToken(err):
		return SendError(c, fiber.StatusUnauthorized, err.Error())
	case errors.IsInvalidVerificationToken(err):
		return SendError(c, fiber.StatusUnauthorized, err.Error())
	case errors.IsPasswordValidation(err):
		return SendError(c, fiber.StatusUnprocessableEntity, err.Error())
