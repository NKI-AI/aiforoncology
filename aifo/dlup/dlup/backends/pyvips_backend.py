...
class PyVipsSlide(AbstractSlideBackend):
    ...
    def __init__(self, filename: PathLike, load_with_openslide: bool = False, rgb: bool = False) -> None:
        """
        Read slide with pyvips.
        ...
        filename : PathLike
            Path to image.
        load_with_openslide : bool, optional
            Whether to load the image with openslide, by default False.
        rgb : bool, optional
            Whether to use RGB format, by default False. There typically is not much information in the alpha channel.
        """
        super().__init__(filename)
        self._filename = str(filename)
        self._rgb = rgb
        if not load_with_openslide:
            self._image = pyvips.Image.new_from_file(self._filename)
            if self._image.get("vips-loader") != "openslideload" and rgb:
                warnings.warn(
                    "The image was loaded with openslide, but could not pass rgb as in VipsForeignLoadTiffFile rgb is not supported."
                )
                self._image = pyvips.Image.openslideload(self._filename)
            if self._image.get("vips-loader") == "openslideload" and rgb:
                # We will need to reopen the image.
                self._image = pyvips.Image.openslideload(self._filename, rgb=self._rgb)
        else:
            self._image = pyvips.Image.openslideload(self._filename, rgb=self._rgb)
    ...
    def _read_as_openslide(self, path: PathLike) -> None:
        ...
        self._images = [
            pyvips.Image.openslideload(str(path), level=level, rgb=self._rgb) for level in range(1, self._level_count)
        ]
        ...
        if mpp_x is not None and mpp_y is not None:
            check_if_mpp_is_valid(mpp_x, mpp_y)
            self._spacings = [
                tuple((np.array([mpp_x, mpp_y]) * downsample).astype(float)) for downsample in self._downsamples
            ]
        ...
        if (
            ...
            offset = (
                int(self._image.get(openslide.PROPERTY_NAME_BOUNDS_X)),
                int(self._image.get(openslide.PROPERTY_NAME_BOUNDS_Y)),
            )
        else:
            offset = (0, 0)
        ...
        if (
            ...
            size = (
                int(self._image.get(openslide.PROPERTY_NAME_BOUNDS_WIDTH)),
                int(self._image.get(openslide.PROPERTY_NAME_BOUNDS_HEIGHT)),
            )
        else:
            size = self._shapes[0]
