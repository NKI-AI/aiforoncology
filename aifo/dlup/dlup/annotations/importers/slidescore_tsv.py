...
def parse_points(data: list[CoordDict], label: str) -> List[Point]:
    ...
    return [Point(item["x"], item["y"], label=label, color=None) for item in data]
...
def parse_rectangle(data: SlideScoreBox, label: str, box_as_polygon: bool = True) -> Union[Box, Polygon]:
...
def parse_annotations(
    annotation: str, label: str, box_as_polygon: bool = False
    ...
    if isinstance(data, list) and all("x" in item and "y" in item for item in data):
        ...
        for item in data:
            if item["type"] == "polygon":
                ...
                parsed_data.append(parse_rectangle(item, label=label, box_as_polygon=box_as_polygon))
        return parsed_data
...
def slidescore_tsv_importer(
    ...
    for row in filtered_rows:
        parsed_annotations = parse_annotations(
            annotation=row["Answer"], label=row["Question"], box_as_polygon=box_as_polygon
        )
