...
def _validate_annotations(
    data_description: Optional[DataDescription], annotations: Optional[SlideAnnotations]
) -> Optional[SlideAnnotations]:
    if annotations is None:
        return None

    if isinstance(annotations, SlideAnnotations):
        if data_description is None:
            raise ValueError(
                "Annotations as a `SlideAnnotations` class are provided but no data description is given."
...
class _ValidationDataset(Dataset[DlupDatasetSample]):
    ...
    def __init__(
        ...
        mask: Optional[SlideAnnotations] = None,
    ...
    def _get_annotation_data(
        ...
        if self._data_description.remap_labels:
            self._annotations.relabel_polygons(self._data_description.remap_labels)

        region = _annotations.polygons.to_mask().numpy()
        if self._data_description.use_roi:
            roi = _annotations.rois.to_mask().numpy()
        else:
            roi = None
...
def _get_uuid_for_filename(input_path: Path) -> str:
    """Generate a short, unique identifier for the given input path.

    Parameters
    ----------
    input_path : Path
        The input path to generate an identifier for.

    Returns
    -------
    str
        A unique identifier for the given input path.
    """
    # Get the absolute path of the file
    input_path = Path(input_path).resolve()
    hash_object = hashlib.md5(str(input_path.parent).encode()).hexdigest()

    return f"{hash_object}_{input_path.stem}"
