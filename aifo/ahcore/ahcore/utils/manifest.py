...
from dlup.data.dataset import RegionFromSlideDataset, SlideDataset, TileSample
from dlup.tiling import GridOrder, TilingMode
from dlup.utils.backends import ImageBackend as DLUPImageBackend
...
_AnnotationReturnTypes = SlideAnnotations | SlideImage
...
def parse_annotations_from_record(
...
) -> SlideAnnotations | SlideImage | None:
    """
    ...
    valid_readers = ["PYVIPS", "TIFFFILE", "OPENSLIDE", "DLUP_XML", "IMAGEIO"]

    reader_name = cast(
        Literal["PYVIPS", "TIFFFILE", "OPENSLIDE", "DLUP_XML", "IMAGEIO"],
        record.reader,
    )
    ...
    if reader_name == "DLUP_XML":
        return SlideAnnotations.from_file_path(annotations_root / record.filename, reader="dlup_xml")

    if reader_name in ["PYVIPS", "TIFFFILE", "OPENSLIDE"]:
        return SlideImage.from_file_path(annotations_root / record.filename, backend=reader_name)

    if reader_name == "IMAGEIO":
        return iio.imread(annotations_root / record.filename)[..., 0]
...
def get_mask_and_annotations_from_record(
    ...
    Returns
    -------
    tuple[SlideAnnotations, SlideAnnotations]
        The mask and annotations.
    """

    if len(record.masks) > 1:
        raise ValueError(f"Multiple masks found for {record.filename}")

    _masks = [parse_annotations_from_record(annotations_root, mask) for mask in record.masks]
    _annotations = parse_annotations_from_record(annotations_root, record.annotation)
    return None if len(_masks) == 0 else _masks[0], _annotations
...
def get_relevant_image_info_from_record(
    ...
    assert (
        isinstance(mask, SlideAnnotations)
        or (mask is None)
        or isinstance(mask, SlideImage)
        or isinstance(mask, np.ndarray)
    )
    mask_threshold = 0.0 if stage != "fit" else data_description.mask_threshold

    if data_description.use_roi:
        logger.warning("Setting use_roi will have no effect as this is currently derived from the annotations!")
    use_rois = annotations.num_rois > 0
    ...
    random_sample_in_grid = getattr(grid_description, "random_sample_in_grid", False)
    ...
    output_dict: ImageInfoDict = {
        ...
        "mask": mask if not use_rois else annotations,
...
def datasets_from_data_description(
    ...
    transform: Callable[[TileSample], RegionFromSlideDataset] | None,
    stage: str,
    error_on_missing: bool = False,
) -> Generator[SlideDataset, None, None]:
    ...
    for _, patient in enumerate(patients):
        ...
        for image in patient.images:
            ...
            random_sample_in_grid = image_info["random_sample_in_grid"]
            mask = image_info["mask"]
            mask_threshold = image_info["mask_threshold"]
            annotations = image_info["annotations"]

            assert isinstance(image_path, Path)
            if error_on_missing and not image_path.exists():
                raise FileNotFoundError(errno.ENOENT, f"No such file: '{image_path}'")
            elif not image_path.exists():
                logger.warning(f"Skipping image {image_path} as it does not exist.")
                continue

            assert isinstance(mpp, float)
            assert (
                isinstance(tile_size, tuple) and len(tile_size) == 2 and all(isinstance(i, int) for i in tile_size)  # pylint: disable=not-an-iterable
            )
            assert (
            ...
            assert isinstance(annotations, SlideAnnotations) or (annotations is None)
            ...
            dataset = SlideDataset.from_standard_tiling(
                ...
                transform=transform,
                backend=functools.partial(backend.value, rgb=True),  # type: ignore
...
def open_db(database_uri: str, ensure_exists: bool = True) -> Session:
    ...
    """
    if database_uri.startswith("sqlite://") and ensure_exists:
        # Check if file exists
        database_path = database_uri.replace("sqlite:///", "")
        if not Path(database_path).exists():
            raise FileNotFoundError(errno.ENOENT, f"No such SQLite database: '{database_uri}'")
