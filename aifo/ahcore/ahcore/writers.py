...
class Writer(abc.ABC):
    ...
    def get_writer_metadata(self, first_batch: GenericNumberArray) -> WriterMetadata:
        ...
        return WriterMetadata(
            ...
            versions={
                "dlup": dlup.__version__,
                "ahcore": ahcore.__version__,
                "ahcore_git_hash": get_git_hash(),
            },
        )
    ...
    def init_writer(self, first_coordinates: GenericNumberArray, first_batch: GenericNumberArray, file: Any) -> Any:
        ...
        elif self._data_format == DataFormat.IMAGE:
            shape = first_batch.shape[1:]
            self._data = self.create_dataset(
                file,
                "data",
...
class ZarrFileImageWriter(Writer):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._store = zarr.storage.LocalStore(str(self._filename.with_suffix(self._partial_suffix)))
    ...
    def create_dataset(
        ...
        """
        compressors = (
            zarr.codecs.BloscCodec(cname="zstd", clevel=3, shuffle=zarr.codecs.BloscShuffle.bitshuffle)
            if compression == "gzip"
            else None
        )
        dataset = file.create_array(
            name, shape=shape, dtype=dtype, chunks=chunks if chunks is not None else "auto", compressors=compressors
        )
        if data is not None:
            dataset[:] = data
        return dataset
