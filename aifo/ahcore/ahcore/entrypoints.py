...
def general_setup(config: DictConfig):
    """General setup for data module, model, augmentations, and loading checkpoint weights."""
    # TODO check if this also work for train
    ...
    # Initialize augmentations (if any)
    augmentations: dict[str, nn.Module] | None = None
    if "augmentations" in config:
        augmentations = {}
        for stage in config.augmentations:
