...
class SlideImageMetaData(BaseModel):
    ...
    @classmethod
    def from_dataset(cls, dataset: SlideDataset) -> "SlideImageMetaData":
...
class DatasetConfigs(BaseModel):
    """Configurations of the SlideDataset dataset"""
...
def _save_thumbnail(
    ...
    _scaling = 1.0
    with SlideImage.from_file_path(image_fn, backend=ImageBackend.PYVIPS) as slide_image:
...
def create_slide_image_dataset(
    ...
    return SlideDataset.from_standard_tiling(
...
def _generator(
    dataset: SlideDataset, quality: int | None, compression: Literal["JPEG", "PNG"] | None
