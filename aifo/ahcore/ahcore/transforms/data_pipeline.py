...
class ModelTransforms(nn.Module):
    def __init__(
        self,
        model: nn.Module,
        ...
        self.register_buffer("_mean", torch.tensor(mean if mean is not None else [0.0, 0.0, 0.0]))
        self.register_buffer("_std", torch.tensor(std if std is not None else [1.0, 1.0, 1.0]))

        # Check that std is not zero
        if torch.any(self._std == 0):
            raise ValueError("Standard deviation cannot be zero")

    def _transform_input(self, sample: torch.Tensor) -> torch.Tensor:
        # Perform normalization
        return (sample / 255.0 - self._mean) / self._std

    def forward(self, sample: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            transformed_sample = self._transform_input(sample["image"])
            return {"features": self.model(transformed_sample), **sample}


class InputNormalizer(nn.Module):
    """
    Normalizes the input image from `[0, 255]` to `[0, 1]` range.
    """

    def __init__(self) -> None:
        super().__init__()

    def forward(self, *args: torch.Tensor, **kwargs: Any) -> Union[list[torch.Tensor], torch.Tensor]:
        data_keys = kwargs.get("data_keys", [DataKey.INPUT] * len(args))
...
class MeanStdNormalizer(nn.Module):
    ...
    def __init__(
        ...
        mean : tuple[float, float, float], optional
            The mean for normalization. Defaults to (0.0, 0.0, 0.0).
        std : tuple[float, float, float], optional
            The standard deviation for normalization. Defaults to (1.0, 1.0, 1.0).
        """
        super().__init__()
        self.register_buffer("_mean", torch.tensor(mean if mean is not None else [0.0, 0.0, 0.0]))
        self.register_buffer("_std", torch.tensor(std if std is not None else [1.0, 1.0, 1.0]))
    ...
    def forward(self, *args: torch.Tensor, **kwargs: Any) -> Union[list[torch.Tensor], torch.Tensor]:
        """
        Normalize the input images using the mean and standard deviation.

        Parameters
        ----------
        *args : torch.Tensor
            Input tensors to normalize.
        **kwargs : dict
            Additional arguments, must contain 'data_keys'.
        ...
        for sample, data_key in zip(args, data_keys):
            if data_key in [DataKey.INPUT, 0, "INPUT"]:
                ...
                sample = (sample - self._mean[..., None, None]) / self._std[..., None, None]
            output.append(sample)

        # Return a single tensor if only one sample was processed
        return output[0] if len(output) == 1 else output
...
class DataPipelineFactory(nn.Module):
    """Factory for the data pipeline. There are three classes of pipeline components:
    ...
    def __init__(
        ...
        initial_transforms = [InputNormalizer()] + initial_transforms if initial_transforms else [InputNormalizer()]
