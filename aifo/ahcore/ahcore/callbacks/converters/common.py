...
class ConvertCallbacks(abc.ABC):
    ...
    @abc.abstractmethod
    def process_task(self, filename: Path, cache_path: Path) -> Any:
        """Abstract method to process the task"""

    def schedule_task(self, filename: Path, cache_path: Path) -> None:
        """Schedule a task for processing"""
        self._task_queue.put((filename, cache_path))  # Put task into the queue for asynchronous processing
    ...
    def worker(self) -> None:
        ...
        while True:
            ...
            result = self.process_task(filename, cache_path)
    ...
    def start(self, filename: str) -> None:
        ...
        assert Path(filename).exists(), f"Input path {filename} does not exist"
        assert cache_path.exists(), f"Cache path {cache_path} does not exist"

        self.schedule_task(filename=Path(filename), cache_path=cache_path)
