...
from ahcore.utils.manifest import (
    ...
    get_mask_and_annotations_from_record,
)
...
class ComputeWsiMetricsCallback(ConvertCallbacks):
    ...
    def process_task(self, filename: Path, cache_path: Path) -> dict[str, str | dict[str, float | int]]:
        ...
        if self._save_per_image:
            cache_path = task_data.cache_path
            output_json_fn = cache_path.with_suffix(".json")
            output_dict = {"filename": str(filename), "cache_path": str(cache_path), "metrics": curr_metrics}
...
def prepare_task_data(
    ...
    cache_path = get_output_filename(
        dump_dir=dump_dir,
    ...
    return WsiMetricTaskData(filename, cache_path, metadata, mask, _annotations)
...
def compute_metrics_for_case(
    ...
    with image_reader(task_data.cache_path, stitching_mode=StitchingMode.CROP) as cache_reader:
        ...
        for sample in dataset_of_validation_image:
            # TODO: Check if this is indeed possible to do this way, sample["prediction"] is suddenly a pyvips Image here.
            prediction = torch.from_numpy(np.asarray(sample["prediction"])).unsqueeze(0).float()
            target = torch.from_numpy(np.asarray(sample["target"])).unsqueeze(0)
            roi = torch.from_numpy(np.asarray(sample["roi"])).unsqueeze(0) if sample["roi"] is not None else None
