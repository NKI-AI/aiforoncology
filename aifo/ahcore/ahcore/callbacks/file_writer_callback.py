...
class WriteFileCallback(AbstractWriterCallback):
    def __init__(
        ...
        dump_dir : pathlib.Path
            The directory to dump the cache files to.
    ...
    def _get_writer_data_args(
        ...
        if data_format == DataFormat.IMAGE or data_format == DataFormat.COMPRESSED_IMAGE:
            ...
            if stage == "validate":
                # TODO: This assumes there is only one grid used!
                # TODO: Verify this assumption
                # TODO: If we are not correct, we might only be using one of the ROIs?
                grid = current_dataset.grids[0][0]  # pylint: disable=protected-access
            else:
            ...
            tile_size = (1, 1)
            tile_overlap = (0, 0)
            grid = current_dataset.grids[0][0]  # give grid, bc doesn't work otherwise
