"""
This module contains the core Lightning module for research. This module is responsible for:
...
def create_images(image, image_mask):
    # Color lut 0 -> (0, 0, 0), 1 -> (0, 0, 255)
    color_lut = np.array([[0, 0, 0], [0, 0, 255]], dtype=np.uint8)
    mask_array = np.array(image_mask)

    # Apply the color LUT to the mask
    mask_array = color_lut[mask_array]

    # Find non-black mask regions
    non_black_mask = np.any(mask_array != 0, axis=2)

    # Create the alpha channel (transparent or semi-transparent based on the mask)
    new_alpha = np.where(non_black_mask, 90, 0).astype(np.uint8)  # 90 is the alpha value

    # Add an extra dimension for the alpha channel
    transparent_mask = np.zeros((mask_array.shape[0], mask_array.shape[1], 4), dtype=np.uint8)
    transparent_mask[:, :, :3] = mask_array  # Copy the RGB values
    transparent_mask[:, :, 3] = new_alpha  # Set the alpha channel

    # Create PIL images and combine them
    transparent_mask_image = PIL.Image.fromarray(transparent_mask, "RGBA")
    image_region_image = PIL.Image.fromarray(np.array(image)).convert("RGBA")
...
class AhCoreLightningModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self._augmentations = torch.nn.ModuleDict(augmentations) if augmentations else None
    ...
    def do_step(self, batch: DlupDatasetSample, batch_idx: int, stage: TrainerFn | str) -> LitModuleSample:
        ...
        if self._loss is None:
            raise RuntimeError(
                f"Loss is not defined for {self.__class__.__name__}. This is required during training and validation"
            )
