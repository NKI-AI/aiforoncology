...
func DefaultConfig() Config {
	return Config{
		...
		Database: DatabaseConfig{
			URL: "mongodb://localhost:27017/slidescope",
		},
		Auth: AuthConfig{
...
// GetEnvDuration gets duration in minutes from environment or uses default
func GetEnvDuration(key string, defaultMinutes int) time.Duration {
	expStr := os.Getenv(key)
	if expStr == "" {
	...
	return time.Duration(expInt) * time.Minute
}
...
func MergeWithEnv(config Config) Config {
	...
	if envDBURL, exists := os.LookupEnv("DATABASE_URL"); exists {
		if envDBURL != "" {
			slog.Info("DATABASE_URL environment variable overrides config",
				"env_value", envDBURL,
				"config_value", config.Database.URL)
			config.Database.URL = envDBURL
		} else {
			slog.Warn("DATABASE_URL environment variable is empty, ignoring")
		}
	}
	...
	if envPort, exists := os.LookupEnv("PORT"); exists {
		if envPort != "" {
			slog.Info("PORT environment variable overrides config",
				"env_value", envPort,
				"config_value", config.Server.Port)
			config.Server.Port = envPort
		} else {
			slog.Warn("PORT environment variable is empty, ignoring")
		}
	}
	...
	if envName, exists := os.LookupEnv("COOKIE_NAME"); exists {
		if envName != "" {
			slog.Info("COOKIE_NAME environment variable overrides config",
				"env_value", envName,
				"config_value", config.Auth.Cookie.Name)
			config.Auth.Cookie.Name = envName
		} else {
			slog.Warn("COOKIE_NAME environment variable is empty, ignoring")
		}
	}

	if envPath, exists := os.LookupEnv("COOKIE_PATH"); exists {
		if envPath != "" {
			slog.Info("COOKIE_PATH environment variable overrides config",
				"env_value", envPath,
				"config_value", config.Auth.Cookie.Path)
			config.Auth.Cookie.Path = envPath
		} else {
			slog.Warn("COOKIE_PATH environment variable is empty, ignoring")
		}
	}
	...
	if httpOnlyEnv, exists := os.LookupEnv("COOKIE_HTTPONLY"); exists {
		slog.Info("COOKIE_HTTPONLY environment variable processed",
	...
	if secureEnv, exists := os.LookupEnv("COOKIE_SECURE"); exists {
		slog.Info("COOKIE_SECURE environment variable processed",
			"env_value", secureEnv,
	...
	if envSameSite, exists := os.LookupEnv("COOKIE_SAMESITE"); exists {
		if envSameSite != "" {
			slog.Info("COOKIE_SAMESITE environment variable overrides config",
				"env_value", envSameSite,
				"config_value", config.Auth.Cookie.SameSite)
			config.Auth.Cookie.SameSite = envSameSite
		} else {
			slog.Warn("COOKIE_SAMESITE environment variable is empty, ignoring")
		}
	}
