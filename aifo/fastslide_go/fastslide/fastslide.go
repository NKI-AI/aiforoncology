...
/*
#cgo CFLAGS: -I../../fastslide/include
#cgo LDFLAGS: -L../../fastslide -lfastslide
...
// CombineSpectralChannelsWithDisplayRanges combines spectral channels using display ranges.
func (img *Image) CombineSpectralChannelsWithDisplayRanges(
	channelMetadata []ChannelMetadata,
	displayRanges []DisplayRange,
) (*Image, error) {
	if img.handle == nil {
		return nil, &FastSlideError{
			Op:      "CombineSpectralChannelsWithDisplayRanges",
			Code:    CodeInvalidState,
			Message: "image is closed",
		}
	}
	...
	if len(channelMetadata) == 0 {
		return nil, &FastSlideError{
			Op:      "CombineSpectralChannelsWithDisplayRanges",
			Code:    CodeInvalidArgument,
			Message: "channel metadata cannot be empty",
		}
	}

	// Convert Go channel metadata to C
	cMetadata := make([]C.FastSlideChannelMetadata, len(channelMetadata))
	cNames := make([]*C.char, len(channelMetadata))
	cBiomarkers := make([]*C.char, len(channelMetadata))

	for i, meta := range channelMetadata {
		cNames[i] = C.CString(meta.Name)
		cBiomarkers[i] = C.CString(meta.Biomarker)

		cMetadata[i].name = cNames[i]
		cMetadata[i].biomarker = cBiomarkers[i]
		cMetadata[i].color.r = C.uint8_t(meta.Color.R)
		cMetadata[i].color.g = C.uint8_t(meta.Color.G)
		cMetadata[i].color.b = C.uint8_t(meta.Color.B)
		cMetadata[i].exposure_time = C.uint32_t(meta.ExposureTime)
		cMetadata[i].signal_units = C.uint32_t(meta.SignalUnits)
	}
	...
	defer func() {
		for i := range cNames {
			...
			C.free(unsafe.Pointer(cBiomarkers[i]))
		}
	}()

	// Convert display ranges to C
	var cRanges []C.FastSlideDisplayRange
	var cRangesPtr *C.FastSlideDisplayRange
	numRanges := 0

	if len(displayRanges) > 0 {
		cRanges = make([]C.FastSlideDisplayRange, len(displayRanges))
		for i, dr := range displayRanges {
			cRanges[i].min_value = C.double(dr.Min)
			cRanges[i].max_value = C.double(dr.Max)
		}
		cRangesPtr = &cRanges[0]
		numRanges = len(displayRanges)
	}

	// Call C function
	C.fastslide_clear_last_error()
	handle := C.fastslide_combine_spectral_channels_with_display_ranges(
		img.handle,
		&cMetadata[0],
		C.int(len(channelMetadata)),
		cRangesPtr,
		C.int(numRanges),
	)
...
// CombineSpectralChannelsWithHistograms combines spectral channels using histograms to compute display ranges.
func (img *Image) CombineSpectralChannelsWithHistograms(
	channelMetadata []ChannelMetadata,
	...
	if img.handle == nil {
		return nil, &FastSlideError{
			Op:      "CombineSpectralChannelsWithHistograms",
			Code:    CodeInvalidState,
			Message: "image is closed",
		}
	}

	if len(channelMetadata) == 0 || len(histograms) == 0 {
		return nil, &FastSlideError{
			Op:      "CombineSpectralChannelsWithHistograms",
			Code:    CodeInvalidArgument,
			Message: "channel metadata and histograms cannot be empty",
		}
	}

	// Convert Go channel metadata to C (same as above)
	cMetadata := make([]C.FastSlideChannelMetadata, len(channelMetadata))
	cNames := make([]*C.char, len(channelMetadata))
	cBiomarkers := make([]*C.char, len(channelMetadata))
	...
	// Convert histograms to C array
	cHistograms := make([]*C.FastSlideHistogram, len(histograms))
	for i, hist := range histograms {
		if hist.handle == nil {
			return nil, &FastSlideError{
				Op:      "CombineSpectralChannelsWithHistograms",
				Code:    CodeInvalidState,
				Message: fmt.Sprintf("histogram %d is closed", i),
			}
		}
		cHistograms[i] = hist.handle
	}

	// Call C function
	C.fastslide_clear_last_error()
	handle := C.fastslide_combine_spectral_channels_with_histograms(
		img.handle,
		&cMetadata[0],
		C.int(len(channelMetadata)),
		&cHistograms[0],
		C.int(len(histograms)),
		C.double(saturation),
	)
