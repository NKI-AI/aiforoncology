...
from aifocore.models.modules import WindowAttention2d


@pytest.mark.parametrize("window_size", [(3, 3), (2, 6), (6, 2)])
def test_relative_position_shapes_different_window_sizes(window_size: Sequence[int]):
    # arrange & act
    num_heads = 3
    window_attn = WindowAttention2d(
        dim=64,
        num_heads=num_heads,
    ...
    # assert
    number_of_biases = ((2 * window_size[0]) - 1) * ((2 * window_size[1]) - 1)
...
def test_forward_different_window_sizes(window_size: Sequence[int]):
    ...
    x = torch.randn((1, window_size[0] * window_size[1], dim))

    # act
    attention_values = window_attn.forward(x)

    # assert
    assert attention_values.shape == (1, window_size[0] * window_size[1], dim)
