...
class VipsInitializer {
 public:
  explicit VipsInitializer(const char* argv0) {
    ...
    // vips_concurrency_set(1);  // TODO(jonasteuwen): This must not be needed.
  }
 ...
 *
 * This function saves a Vips image to a file. If an options pointer is provided,
 * the image will be saved with the specified options. Otherwise, the image will
 * be saved with the default options.
 * Wrapper to vips::VImage::write_to_file to separate error handling from the
 * rest of the code.
 *
