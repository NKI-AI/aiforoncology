 ...
 * using average downsampling filters
 * 
 ...
 */
absl::Status SaveCompleteLevelForSpectralAverage(
  ...
  for (int level = 0; level < reader.GetLevelCount(); ++level) {
    ...
    if (width < 5000 && height < 5000 && target_level == -1) {
      target_level = level;
      target_width = width;
  ...
  std::filesystem::path filename = output_dir / (level_name + "_original.png");
  ...
  // Now work on the downsampled versions using average filtering
  std::cout << "  Creating downsampled versions using average filtering:\n";

  // Test both 2x and 4x downsampling
  std::vector<uint32_t> factors = {2, 4, 8};

  for (uint32_t factor : factors) {
    std::cout << "    Factor " << factor << "x downsampling ("
              << image.GetWidth() << "x" << image.GetHeight() << " → "
              << image.GetWidth() / factor << "x" << image.GetHeight() / factor
              << ")";

    // Time the resampling operation - run 10 times for average
    constexpr int kNumRuns = 25;
    std::vector<std::chrono::milliseconds> durations;
    durations.reserve(kNumRuns);
    ...
    for (int run = 0; run < kNumRuns; ++run) {
      ...
      try {
        std::unique_ptr<Image> temp_resampled;
        if (factor == 2) {
          temp_resampled = fastslide::resample::Average2x2Resample(image);
        } else if (factor == 4) {
          temp_resampled = fastslide::resample::Average4x4Resample(image);
        } else {
          temp_resampled = fastslide::resample::AverageResample(image, factor);
        }
    ...
    auto average_duration = total_duration / kNumRuns;
    ...
    std::string downsampled_level_name =
        "level_" + std::to_string(target_level) + "_average_" +
        std::to_string(factor) + "x";
    std::filesystem::path downsampled_filename =
        output_dir / (downsampled_level_name + ".png");

    std::cout << "      Saving " << downsampled_filename.filename().string();
    if (!SaveAsPNG(*resampled_image, downsampled_filename.string())) {
      std::cout << " → FAILED\n";
      return absl::Status(absl::StatusCode::kInternal,
                          "Failed to save downsampled PNG");
    }
    std::cout << " ✓\n";
  }
...
int main(int argc, char** argv) {
  auto config = fastslide::examples::ParseCliAndInit(
      argc, argv,
      "FastSlide Spectral Average Example - Multiplex/spectral slide "
      "processing with average downsampling filters");
  ...
  auto histograms = ComputeChannelHistograms(*reader);
  ...
  // Save main level with spectral processing using average downsampling
  auto status = SaveCompleteLevelForSpectralAverage(
      *reader, config.output_dir, channel_metadata, display_ranges);
  if (!status.ok()) {
