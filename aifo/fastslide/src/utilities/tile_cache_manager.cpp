...
absl::StatusOr<std::shared_ptr<CachedTile>> TileCacheManager::GetTile(
  ...
  if (cached_tile) {
    // LOG(INFO) << "[Perf] TileCache hit level=" << level
    //           << " x=" << tile_coords[0] << " y=" << tile_coords[1];
    return cached_tile;
  }

  // Cache miss - load the tile using the provided loader
  // LOG(INFO) << "[Perf] TileCache miss level=" << level
  //           << " x=" << tile_coords[0] << " y=" << tile_coords[1];
...
std::shared_ptr<CachedTile> TileCacheManager::GetTileFromCache(
  ...
  TileKey key = CreateCacheKey(filename, level, tile_x, tile_y);
...
TileKey TileCacheManager::CreateCacheKey(const std::string& filename,
                                         uint16_t level, uint32_t tile_x,
                                         uint32_t tile_y) const {
  return TileKey{
      .filename = filename, .level = level, .tile_x = tile_x, .tile_y = tile_y};
}
