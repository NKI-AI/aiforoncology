...
TIFFHandleGuard TIFFHandlePool::Acquire(std::chrono::milliseconds timeout) {
  ScopedTimer t("TIFFHandlePool::Acquire");
  ...
  if (!permit_acquired) {
    LOG(INFO) << "Failed to acquire permit, returning nullptr";
    return {nullptr, this};
  }
  ...
  if (shutdown_flag_.load()) {
    ...
    LOG(INFO)
        << "Pool shutting down after permit acquisition, releasing permit";
    semaphore_.release();
    return {nullptr, this};
  }
  ...
  if (total_handles_opened_.load() < max_pool_size_) {
    ...
    LOG(INFO) << "Failed to open new handle, rolling back reservation";
...
TIFFHandleGuard TIFFHandlePool::TryAcquire() {
  ScopedTimer t("TIFFHandlePool::TryAcquire");
  // Check shutdown flag first
  if (shutdown_flag_.load()) {
...
void TIFFHandlePool::Release(TIFF* handle) {
  ...
  if (shutdown_flag_.load()) {
    LOG(INFO) << "Pool shutting down, closing handle directly";
    TIFFClose(handle);
    return;
  }
