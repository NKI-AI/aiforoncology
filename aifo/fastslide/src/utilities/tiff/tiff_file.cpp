...
absl::StatusOr<TiffFile> TiffFile::Create(TIFFHandlePool* pool) {
  ScopedTimer t("TiffFile::Create (Acquire)");
  if (pool == nullptr) {
...
absl::Status TiffFile::ValidateFile(const std::filesystem::path& filename) {
  ScopedTimer t("TiffFile::ValidateFile (open/close)");
...
absl::Status TiffFile::SetDirectory(uint16_t dir_index) {
  // Timer disabled, it takes about ~100us, so not a bottleneck.
  // ScopedTimer t("TiffFile::SetDirectory");
  if (!IsValid()) {
    return MAKE_STATUS(absl::StatusCode::kFailedPrecondition,
                       "TIFF handle is not valid");
  }
...
absl::StatusOr<TiffImageDimensions> TiffFile::GetImageDimensions() const {
  auto width_result = GetRequiredField<uint32_t>(TIFFTAG_IMAGEWIDTH);
  if (!width_result.ok()) {
    return width_result.status();
  }
  auto height_result = GetRequiredField<uint32_t>(TIFFTAG_IMAGELENGTH);
  if (!height_result.ok()) {
    return height_result.status();
  }
  return TiffImageDimensions{width_result.value(), height_result.value()};
}
...
absl::StatusOr<TiffTileDimensions> TiffFile::GetTileDimensions() const {
  ...
  auto width_result = GetRequiredField<uint32_t>(TIFFTAG_TILEWIDTH);
  if (!width_result.ok()) {
    return width_result.status();
  }
  auto height_result = GetRequiredField<uint32_t>(TIFFTAG_TILELENGTH);
  if (!height_result.ok()) {
    return height_result.status();
  }
  return TiffTileDimensions{width_result.value(), height_result.value()};
}
...
absl::StatusOr<TiffPhotometric> TiffFile::GetPhotometric() const {
  auto result = GetRequiredField<uint16_t>(TIFFTAG_PHOTOMETRIC);
  if (!result.ok()) {
    return result.status();
  }
  return static_cast<TiffPhotometric>(result.value());
}
...
absl::StatusOr<DataType> TiffFile::GetDataType() const {
  auto bits_per_sample_result = GetBitsPerSample();
  if (!bits_per_sample_result.ok()) {
    return bits_per_sample_result.status();
  }

  auto sample_format_result = GetSampleFormat();
  if (!sample_format_result.ok()) {
    return sample_format_result.status();
  }

  const uint16_t bits_per_sample = bits_per_sample_result.value();
  const TiffSampleFormat sample_format = sample_format_result.value();
...
absl::Status TiffFile::ReadTile(void* buffer,
  ...
  ScopedTimer t("TiffFile::ReadTile (decoder)");
  if (!IsValid()) {
    return MAKE_STATUS(absl::StatusCode::kFailedPrecondition,
                       "TIFF handle is not valid");
  }
...
absl::Status TiffFile::ReadScanline(void* buffer, uint32_t row,
                                    uint16_t sample) const {
  // Timer disabled, it takes about 10us - 72us, so not a bottleneck.
  // ScopedTimer t("TiffFile::ReadScanline");
  if (!IsValid()) {
    return MAKE_STATUS(absl::StatusCode::kFailedPrecondition,
                       "TIFF handle is not valid");
  }
