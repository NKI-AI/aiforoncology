...
class TileCacheTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    // Create test keys
    key1_ =
        TileKey{.filename = "test1.tiff", .level = 0, .tile_x = 0, .tile_y = 0};
    key2_ =
        TileKey{.filename = "test1.tiff", .level = 0, .tile_x = 1, .tile_y = 0};
    key3_ =
        TileKey{.filename = "test2.tiff", .level = 1, .tile_x = 0, .tile_y = 0};
  }
...
TEST_F(TileCacheTest, TileKeyOperations) {
  TileKey key1{
      .filename = "test.tiff", .level = 0, .tile_x = 100, .tile_y = 200};
  TileKey key2{
      .filename = "test.tiff", .level = 0, .tile_x = 100, .tile_y = 200};
  TileKey key3{
      .filename = "test.tiff", .level = 0, .tile_x = 100, .tile_y = 201};
  TileKey key4{
      .filename = "test.tiff", .level = 1, .tile_x = 100, .tile_y = 200};
  TileKey key5{
      .filename = "other.tiff", .level = 0, .tile_x = 100, .tile_y = 200};
  ...
  for (int i = 0; i < num_threads; ++i) {
    threads.emplace_back([&cache, &successful_operations, i]() {
      for (int j = 0; j < operations_per_thread; ++j) {
        ...
        TileKey key{.filename = "thread_" + std::to_string(i) + "_op_" +
                                std::to_string(j) + ".tiff",
                    .level = static_cast<uint16_t>(i % 5),
                    .tile_x = static_cast<uint32_t>(j),
                    .tile_y = static_cast<uint32_t>(i)};
