...
template <class T, std::size_t N, xt::layout_type L>
struct type_caster<xt::xexpression<xt::pytensor<T, N, L>>>
    : pyobject_caster<xt::pytensor<T, N, L>> {
...
class pytensor : public pycontainer<pytensor<T, N, L>>,
 ...
 public:
  ...
  explicit pytensor(const shape_type& shape, const_reference value,
                    layout_type l = layout_type::row_major);
...
/**
     ...
     */
template <class T, std::size_t N, layout_type L>
inline pytensor<T, N, L>::pytensor() : base_type() {
...
template <class T, std::size_t N, layout_type L>
inline pytensor<T, N, L>::pytensor(pybind11::handle h,
...
template <class T, std::size_t N, layout_type L>
inline pytensor<T, N, L>::pytensor(pybind11::handle h,
                                   pybind11::object::stolen_t s)
    : base_type(h, s) {
  init_from_python();
}

template <class T, std::size_t N, layout_type L>
inline pytensor<T, N, L>::pytensor(const pybind11::object& o) : base_type(o) {
  init_from_python();
}
...
inline pytensor<T, N, L>::pytensor(const shape_type& shape,
                                   const_reference value, layout_type l) {
  compute_strides(shape, l, m_strides);
  init_tensor(shape, m_strides);
...
template <class T, std::size_t N, layout_type L>
template <class E>
inline pytensor<T, N, L>::pytensor(const xexpression<E>& e) : base_type() {
  shape_type shape =
...
template <class T, std::size_t N, layout_type L>
template <class E>
inline auto pytensor<T, N, L>::operator=(const xexpression<E>& e)
    -> self_type& {
...
template <class T, std::size_t N, layout_type L>
inline void pytensor<T, N, L>::init_tensor(const shape_type& shape,
                                           const strides_type& strides) {
  ...
  auto tmp = pybind11::reinterpret_steal<pybind11::object>(PyArray_NewFromDescr(
      &PyArray_Type, (PyArray_Descr*)dtype.release().ptr(),
      static_cast<int>(shape.size()), const_cast<npy_intp*>(shape.data()),
      python_strides.value, nullptr, flags, nullptr));
...
inline void pytensor<T, N, L>::init_from_python() {
  ...
  if (PyArray_NDIM(this->python_array()) != N) {
    throw std::runtime_error(
        "NumPy: ndarray has incorrect number of dimensions");
  }

  std::copy(PyArray_DIMS(this->python_array()),
            PyArray_DIMS(this->python_array()) + N, m_shape.begin());
  std::transform(PyArray_STRIDES(this->python_array()),
                 PyArray_STRIDES(this->python_array()) + N, m_strides.begin(),
...
template <class T, std::size_t N, layout_type L>
inline auto pytensor<T, N, L>::shape_impl() const noexcept
...
template <class T, std::size_t N, layout_type L>
inline auto pytensor<T, N, L>::strides_impl() const noexcept
    -> const inner_strides_type& {
  return m_strides;
}

template <class T, std::size_t N, layout_type L>
inline auto pytensor<T, N, L>::backstrides_impl() noexcept
    -> inner_backstrides_type& {
  return m_backstrides;
}

template <class T, std::size_t N, layout_type L>
inline auto pytensor<T, N, L>::backstrides_impl() const noexcept
...
template <class T, std::size_t N, layout_type L>
inline auto pytensor<T, N, L>::storage_impl() const noexcept
    -> const storage_type& {
  return m_storage;
}
}  // namespace xt
