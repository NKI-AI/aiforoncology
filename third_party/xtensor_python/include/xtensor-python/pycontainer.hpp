...
/**
     ...
     */
template <class D>
class pycontainer : public pybind11::object, public xcontainer<D> {
 public:
...
template <class T>
struct numpy_traits<T, std::enable_if_t<pybind11::detail::satisfies_any_of<
                           T, std::is_arithmetic, xtl::is_complex>::value>> {
 private:
  ...
  constexpr static const int value_list[15] = {
      ...
      NPY_INT32,  NPY_UINT32,     NPY_INT64,  NPY_UINT64,  NPY_FLOAT,
      NPY_DOUBLE, NPY_LONGDOUBLE, NPY_CFLOAT, NPY_CDOUBLE, NPY_CLONGDOUBLE};

 public:
  using value_type = std::remove_const_t<T>;

  static constexpr int type_num =
      value_list[pybind11::detail::is_fmt_numeric<value_type>::index];
};
...
bool check_array_type(const pybind11::handle& src, std::true_type) {
  ...
  return xt::detail::pyarray_type(
             reinterpret_cast<PyArrayObject*>(src.ptr())) == type_num;
}
...
bool check_array(const pybind11::handle& src) {
  using is_arithmetic_type = std::integral_constant<
      bool, bool(pybind11::detail::satisfies_any_of<T, std::is_arithmetic,
                                                    xtl::is_complex>::value)>;
...
template <class D>
inline pycontainer<D>::pycontainer() : pybind11::object() {}
...
inline PyObject* pycontainer<D>::raw_array_t(PyObject* ptr) {
  ...
  auto res =
      PyArray_FromAny(ptr, (PyArray_Descr*)dtype.release().ptr(), 0, 0,
...
inline auto pycontainer<D>::get_buffer_size() const -> size_type {
  const size_type& (*min)(const size_type&, const size_type&) =
      std::min<size_type>;
  size_type min_stride =
      this->strides().empty()
          ? size_type(1)
          : std::max(size_type(1),
                     std::accumulate(
                         this->strides().cbegin(), this->strides().cend(),
...
template <class S>
struct check_dims {
  static bool run(std::size_t) { return true; }
};
...
struct check_dims<std::array<T, N>> {
  static bool run(std::size_t new_dim) {
    if (new_dim != N) {
      ...
      err_msg << "Invalid conversion to pycontainer, expecting a container of "
                 "dimension "
...
inline void pycontainer<D>::resize(const S& shape) {
  if (shape.size() != this->dimension() ||
      !std::equal(std::begin(shape), std::end(shape),
                  std::begin(this->shape()))) {
...
inline void pycontainer<D>::resize(const S& shape, layout_type l) {
  strides_type strides =
      xtl::make_sequence<strides_type>(shape.size(), size_type(1));
...
inline void pycontainer<D>::resize(const S& shape,
  ...
  derived_type tmp(xtl::forward_sequence<shape_type, decltype(shape)>(shape),
                   strides);
...
inline auto& pycontainer<D>::reshape(S&& shape, layout_type layout) & {
  if (compute_size(shape) != this->size()) {
    throw std::runtime_error(
        "Cannot reshape with incorrect number of elements (" +
        std::to_string(this->size()) + " vs " +
