...
template <typename T, xt::layout_type L>
struct type_caster<xt::xexpression<xt::pyarray<T, L>>>
    : pyobject_caster<xt::pyarray<T, L>> {
...
class pyarray : public pycontainer<pyarray<T, L>>,
 ...
 public:
  ...
  explicit pyarray(const shape_type& shape, const_reference value,
                   layout_type l = layout_type::row_major);
...
//@{
template <class T, layout_type L>
inline pyarray<T, L>::pyarray() : base_type() {
...
/**
     ...
     */
template <class T, layout_type L>
inline pyarray<T, L>::pyarray(const value_type& t) : base_type() {
  base_type::resize(xt::shape<shape_type>(t), default_dynamic_layout());
  nested_copy(m_storage.begin(), t);
}

template <class T, layout_type L>
inline pyarray<T, L>::pyarray(nested_initializer_list_t<T, 1> t) : base_type() {
  base_type::resize(xt::shape<shape_type>(t), default_dynamic_layout());
...
template <class T, layout_type L>
inline pyarray<T, L>::pyarray(nested_initializer_list_t<T, 2> t) : base_type() {
  base_type::resize(xt::shape<shape_type>(t), default_dynamic_layout());
  L == layout_type::row_major
      ? nested_copy(m_storage.begin(), t)
      : nested_copy(this->template begin<layout_type::row_major>(), t);
...
template <class T, layout_type L>
inline pyarray<T, L>::pyarray(nested_initializer_list_t<T, 3> t) : base_type() {
  base_type::resize(xt::shape<shape_type>(t), default_dynamic_layout());
  L == layout_type::row_major
      ? nested_copy(m_storage.begin(), t)
      : nested_copy(this->template begin<layout_type::row_major>(), t);
...
template <class T, layout_type L>
inline pyarray<T, L>::pyarray(nested_initializer_list_t<T, 4> t) : base_type() {
  base_type::resize(xt::shape<shape_type>(t), default_dynamic_layout());
  L == layout_type::row_major
      ? nested_copy(m_storage.begin(), t)
      : nested_copy(this->template begin<layout_type::row_major>(), t);
...
template <class T, layout_type L>
inline pyarray<T, L>::pyarray(nested_initializer_list_t<T, 5> t) : base_type() {
  base_type::resize(xt::shape<shape_type>(t), default_dynamic_layout());
  L == layout_type::row_major
      ? nested_copy(m_storage.begin(), t)
      : nested_copy(this->template begin<layout_type::row_major>(), t);
...
template <class T, layout_type L>
inline pyarray<T, L>::pyarray(pybind11::handle h,
                              pybind11::object::borrowed_t b)
    : base_type(h, b) {
  init_from_python();
}
...
template <class T, layout_type L>
inline pyarray<T, L>::pyarray(const pybind11::object& o) : base_type(o) {
  init_from_python();
}
...
/**
     ...
     */
template <class T, layout_type L>
inline pyarray<T, L>::pyarray(const shape_type& shape, const_reference value,
                              layout_type l)
    : base_type() {
  strides_type strides(shape.size());
  compute_strides(shape, l, strides);
...
inline pyarray<T, L>::pyarray(const self_type& rhs)
  ...
  init_from_python();
  std::copy(rhs.storage().cbegin(), rhs.storage().cend(),
            this->storage().begin());
}
...
template <class T, layout_type L>
template <class E>
inline pyarray<T, L>::pyarray(const xexpression<E>& e) : base_type() {
  // TODO: prevent intermediary shape allocation
  shape_type shape =
      xtl::forward_sequence<shape_type, decltype(e.derived_cast().shape())>(
          e.derived_cast().shape());
  strides_type strides =
      xtl::make_sequence<strides_type>(shape.size(), size_type(0));
...
template <class T, layout_type L>
inline void pyarray<T, L>::init_array(const shape_type& shape,
                                      const strides_type& strides) {
  ...
  npy_intp* shape_data =
      reinterpret_cast<npy_intp*>(const_cast<size_type*>(shape.data()));
  ...
  auto tmp = pybind11::reinterpret_steal<pybind11::object>(
      PyArray_NewFromDescr(&PyArray_Type, (PyArray_Descr*)dtype.release().ptr(),
                           static_cast<int>(shape.size()), shape_data,
                           strides_data, nullptr, flags, nullptr));
...
inline void pyarray<T, L>::init_from_python() {
  ...
  if (L != layout_type::dynamic &&
      !do_strides_match(m_shape, m_strides, L, 1)) {
    throw std::runtime_error(
        "NumPy: passing container with bad strides for layout (is it a "
        "view?).");
  }
...
template <class T, layout_type L>
inline auto pyarray<T, L>::shape_impl() const noexcept
    -> const inner_shape_type& {
  return m_shape;
}

template <class T, layout_type L>
inline auto pyarray<T, L>::strides_impl() const noexcept
    -> const inner_strides_type& {
  return m_strides;
}

template <class T, layout_type L>
inline auto pyarray<T, L>::backstrides_impl() const noexcept
...
template <class T, layout_type L>
inline auto pyarray<T, L>::storage_impl() const noexcept
    -> const storage_type& {
  return m_storage;
}
