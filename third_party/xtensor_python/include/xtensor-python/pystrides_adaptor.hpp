...
class pystrides_iterator {
 public:
  ...
  inline pystrides_iterator(pointer current, shape_pointer shape)
      : p_current(current), p_shape(shape) {}
...
template <std::size_t N>
inline pystrides_adaptor<N>::pystrides_adaptor(const_pointer data,
                                               size_type size, shape_type shape)
...
template <std::size_t N>
inline auto pystrides_adaptor<N>::operator[](size_type i) const
    -> const_reference {
