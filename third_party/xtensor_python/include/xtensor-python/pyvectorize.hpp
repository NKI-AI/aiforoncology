...
struct pyvectorizer {
  ...
  template <class F, class = std::enable_if_t<
                         !std::is_same<std::decay_t<F>, pyvectorizer>::value>>
  pyvectorizer(F&& func) : m_vectorizer(std::forward<F>(func)) {}
...
inline auto pyvectorize(F&& f)
    -> decltype(pyvectorize(std::forward<F>(f),
                            (detail::get_function_type<F>*)nullptr)) {
  return pyvectorize(std::forward<F>(f),
                     (detail::get_function_type<F>*)nullptr);
}
