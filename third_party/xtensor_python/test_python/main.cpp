...
xt::xtensor_fixed<int, xt::xshape<4, 3, 2>> example3_xfixed3(
    const xt::xtensor_fixed<int, xt::xshape<2, 3, 4>>& m) {
  return xt::transpose(m) + 2;
}

xt::xtensor_fixed<int, xt::xshape<3, 2>> example3_xfixed2(
    const xt::xtensor_fixed<int, xt::xshape<2, 3>>& m) {
  return xt::transpose(m) + 2;
}

xt::xtensor_fixed<int, xt::xshape<3, 2>, xt::layout_type::column_major>
example3_xfixed2_colmajor(
    const xt::xtensor_fixed<int, xt::xshape<2, 3>,
                            xt::layout_type::column_major>& m) {
  return xt::transpose(m) + 2;
}
...
// Operator examples
//
xt::pyarray<double> array_addition(const xt::pyarray<double>& m,
                                   const xt::pyarray<double>& n) {
  return m + n;
}

xt::pyarray<double> array_subtraction(xt::pyarray<double>& m,
                                      xt::pyarray<double>& n) {
  return m - n;
}

xt::pyarray<double> array_multiplication(xt::pyarray<double>& m,
                                         xt::pyarray<double>& n) {
  return m * n;
}

xt::pyarray<double> array_division(xt::pyarray<double>& m,
                                   xt::pyarray<double>& n) {
  return m / n;
}
...
void row_major_tensor(
    xt::pytensor<double, 3, xt::layout_type::row_major>& arg) {
...
void col_major_array(xt::pyarray<double, xt::layout_type::column_major>& arg) {
  if (!std::is_same<
          decltype(arg.template begin<xt::layout_type::column_major>()),
          double*>::value) {
    throw std::runtime_error("TEST FAILED");
  }
}
...
PYBIND11_MODULE(xtensor_python_test, m) {
  ...
  m.def("rect_to_polar",
        xt::pyvectorize([](complex_t x) { return std::abs(x); }));

  m.def("compare_shapes",
  ...
  py::class_<C>(m, "C")
      .def(py::init<>())
      .def_property_readonly("copy", [](C& self) { return self.array(); })
      .def_property_readonly(
          "ref", [](C& self) -> C::array_type& { return self.array(); });
  ...
  py::class_<test_native_casters>(m, "test_native_casters")
      .def(py::init<>())
      .def("get_array", &test_native_casters::get_array,
           py::return_value_policy::
               reference_internal)  // memory managed by the class instance
      .def(
          "get_strided_view", &test_native_casters::get_strided_view,
          py::keep_alive<
              0,
              1>())  // keep_alive<0, 1>() => do not free "self" before the returned view
      .def(
          "get_array_adapter", &test_native_casters::get_array_adapter,
          py::keep_alive<
              0,
              1>())  // keep_alive<0, 1>() => do not free "self" before the returned adapter
      .def(
          "get_tensor_adapter", &test_native_casters::get_tensor_adapter,
          py::keep_alive<
              0,
              1>())  // keep_alive<0, 1>() => do not free "self" before the returned adapter
      .def(
          "get_owning_array_adapter",
          &test_native_casters::
              get_owning_array_adapter)  // auto memory management as the adapter owns its memory
      .def(
          "view_keep_alive_member_function",
          [](test_native_casters& self,
             xt::pyarray<double>&
                 a)  // keep_alive<0, 2>() => do not free second parameter before the returned view
          {
            return xt::reshape_view(a, {
                                           a.size(),
                                       });
          },
          py::keep_alive<0, 2>());
