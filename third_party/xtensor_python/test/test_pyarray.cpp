...
TEST(pyarray, initializer_constructor) {
  pyarray<int> r{{{0, 1, 2}, {3, 4, 5}, {6, 7, 8}},
...
TEST(pyarray, from_shape) {
  ...
  EXPECT_TRUE(
      std::equal(arr.shape().begin(), arr.shape().end(), exp_shape.begin()));
...
TEST(pyarray, reshape) {
  ...
  EXPECT_TRUE(std::equal(sc1.begin(), sc1.end(), a.shape().begin()) &&
              a.shape().size() == 2);
  EXPECT_EQ(ptr, a.data());
  a.reshape({6});
  std::vector<std::size_t> sc2 = {6};
  EXPECT_TRUE(std::equal(sc2.begin(), sc2.end(), a.shape().begin()) &&
              a.shape().size() == 1);
  EXPECT_EQ(ptr, a.data());
}
