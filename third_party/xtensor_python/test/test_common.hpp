...
void compare_shape(V& vec, const R& result, bool compare_layout = true) {
  EXPECT_TRUE(std::equal(vec.shape().cbegin(), vec.shape().cend(),
                         result.shape().cbegin()));
  EXPECT_TRUE(std::equal(vec.strides().cbegin(), vec.strides().cend(),
                         result.strides().cbegin()));
// TODO: check why this does not build on modern MSVC compilers
#ifndef WIN32
  EXPECT_TRUE(std::equal(vec.backstrides().cbegin(), vec.backstrides().cend(),
                         result.backstrides().cbegin()));
#endif
...
void test_transpose(V& vec) {
  ...
  using strides_type = typename V::strides_type;
  {
    ...
    EXPECT_TRUE(
        std::equal(vt.shape().cbegin(), vt.shape().cend(), shape_new.cbegin()));
  }

  {
    ...
    EXPECT_TRUE(std::equal(vec.storage().cbegin(), vec.storage().cend(),
                           rm.m_data.cbegin()));
    ...
    std::reverse(shape_new.begin(), shape_new.end());
    EXPECT_EQ(vt.shape(), shape_new);
    EXPECT_TRUE(std::equal(vt.storage().cbegin(), vt.storage().cend(),
                           rm.m_data.cbegin()));

    strides_type new_strides = {rm.m_strides[2], rm.m_strides[1],
                                rm.m_strides[0]};
    EXPECT_EQ(vt.strides(), new_strides);

    strides_type new_backstrides = {rm.m_backstrides[2], rm.m_backstrides[1],
                                    rm.m_backstrides[0]};
...

  {
    ...
    EXPECT_TRUE(
        std::equal(vt.shape().cbegin(), vt.shape().cend(), shape_new.begin()));
    EXPECT_TRUE(std::equal(vt.storage().cbegin(), vt.storage().cend(),
                           rm.m_data.cbegin()));

    strides_type new_strides = {rm.m_strides[1], rm.m_strides[0],
                                rm.m_strides[2]};
    EXPECT_EQ(vt.strides(), new_strides);
...

  {
    ...
    EXPECT_THROW(transpose(vec, {1, 1, 0}, check_policy::full()),
                 transpose_error);
    EXPECT_THROW(transpose(vec, {1, 0, 2, 3}, check_policy::full()),
                 transpose_error);
    EXPECT_THROW(transpose(vec, {1, 2}, check_policy::full()), transpose_error);
    EXPECT_THROW(transpose(vec, {3, 0, 1}, check_policy::full()),
                 transpose_error);
  }
}
...
void test_element(V& vec) {
  {
    ...
    EXPECT_EQ(vec.element(index1.begin(), index1.end()),
              vec.element(index2.begin(), index2.end()));
    EXPECT_EQ(vec.element(index3.begin(), index3.end()),
              vec.element(index4.begin(), index4.end()));
    test_bound_check(vec);
  }
...
void test_iterator(VRM& vecrm, VCM& veccm) {
  {
    ...
    std::copy(rm.data().cbegin(), rm.data().cend(),
              vecrm.template begin<layout_type::row_major>());
    EXPECT_TRUE(std::equal(rm.data().cbegin(), rm.data().cend(),
                           vecrm.storage().cbegin()));
...

  {
    ...
    std::copy(cm.data().cbegin(), cm.data().cend(),
              veccm.template begin<layout_type::column_major>());
    EXPECT_TRUE(std::equal(cm.data().cbegin(), cm.data().cend(),
                           veccm.storage().cbegin()));
...
void test_xiterator(V& vec) {
  ...
  // column shaped_xiterator
  {
    ...
    shape[0] = 2;
    auto iter =
        vec.template begin<shape_type, layout_type::column_major>(shape);
    auto iter_end =
        vec.template end<shape_type, layout_type::column_major>(shape);
    for (size_t i = 0; i < 2 * nb_iter; ++i) {
      ++iter;
    }
    EXPECT_EQ(vec(0, 0, 2), *iter);
...
void test_reverse_xiterator(V& vec) {
  ...
  // shaped_xiterator
  {
    ...
    auto iter_end =
        vec.template rend<shape_type, layout_type::row_major>(shape);
    for (size_t i = 0; i < 2 * nb_iter; ++i) {
      ++iter;
    }
