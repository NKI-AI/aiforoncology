...
decide _how_ will the API be used (if preview is requested, the pixel format
...
no parameters, or better with a parameter with the path to a _directory_
...
In MSan builds it is considered an error to _use_ uninitialized memory. Using
