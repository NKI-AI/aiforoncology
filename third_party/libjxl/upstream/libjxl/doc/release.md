...
pull requests are merged. Several slower checks only run _after_ the code has
...
Release tags are cut from _release branches_. Each MAJOR.MINOR version has its
...
having a release branch allows us to cherry-pick _bug fixes_ from the `main`
branch into the release branch without including the new _features_ from `main`.
...
Release branches _may_ continue to be maintained even after the next minor or
...
in our repository, however you can push to a protected branch when _creating_ it
...
Finally, upload your changes to _your fork_ like normal, except that when
...
- a GitHub "release": this is a GitHub-only concept and is not represented in
  ...
  it _also_ contains release notes and optional binary files attached to the
  release.
...
- The branch v0.7.x will be pushed to gitlab automatically, but make sure to
  manually push the _tag_ of the release also to
