...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        if isinstance(embed_layer, str):
            match embed_layer:
                ...
                case _:
                    raise ValueError("PatchEmbedding type was unrecognized.")
        else:
            self.patch_embed = PatchEmbed(
                img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim
            )
    ...
    def _interpolate_pos_encoding(self, x: torch.Tensor, img_width: int, img_height: int) -> torch.Tensor:
        previous_dtype = x.dtype
        npatch = x.shape[1] - 1
        N = self.pos_embed.shape[1] - 1
        if npatch == N and img_width == img_height:
            return self.pos_embed
        pos_embed = self.pos_embed.float()
        class_pos_embed = pos_embed[:, 0]
        patch_pos_embed = pos_embed[:, 1:]
        dim = x.shape[-1]
        w0 = img_width // self.patch_size
        h0 = img_height // self.patch_size
    ...
    def _prepare_tokens_with_masks(self, x: torch.Tensor, masks: torch.Tensor | None = None) -> torch.Tensor:
        ...
        x = x + self._interpolate_pos_encoding(x, img_width, img_height)
    ...
    def forward_features(
        self, x: torch.Tensor | list[torch.Tensor], masks: torch.Tensor | list[torch.Tensor] = None
