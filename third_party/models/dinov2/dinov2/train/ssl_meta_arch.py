...
class SSLMetaArch(nn.Module):
    def __init__(
        self, cfg: DictConfig, builder_fn: Callable[[DictConfig], DinoVisionTransformer] = build_model_from_cfg
    ) -> None:
        super().__init__()
        self.cfg = cfg
        ...
        student_backbone, teacher_backbone, embed_dim = builder_fn(cfg)
    ...
    def forward_backward(
        self, images: dict[str, torch.Tensor], teacher_temp: float, accumulation_steps: int = 1
    ) -> dict[str, int]:
        n_global_crops = 2
        ...
        loss_accumulator /= accumulation_steps
